...
def initial_population(pop_size):
    first = np.loadtxt('initsols\A1SAVINGS.txt',dtype=int,delimiter='-')
    first = [i for i in first]
    second = np.loadtxt('initsols\A1CHRISTOFIDES.txt',dtype=int,delimiter='-')
    second = [i for i in second]
    third = np.loadtxt('initsols\A1PCA.txt',dtype=int,delimiter='-')
...
def memetic_algorithm(pop_size, num_generations, crossover_prob,mut_prob, tournament_size):
    Gen = []
    BG = []
    BO = []
    BS = []
...
if __name__ == '__main__':
    ...
    best_solution = memetic_algorithm(pop_size=25, num_generations=20, crossover_prob=0.7,mut_prob=0.3,tournament_size=3)
